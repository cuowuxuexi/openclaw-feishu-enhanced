...
async function convertMarkdown(client: Lark.Client, markdown: string, userAccessToken?: string) {
...
async function readDoc(client: Lark.Client, docToken: string, userAccessToken?: string) {
  const opts = userTokenOptions(userAccessToken);
  const [contentRes, infoRes, blocksRes] = await Promise.all([
    client.docx.document.rawContent({ path: { document_id: docToken } }, opts),
    client.docx.document.get({ path: { document_id: docToken } }, opts),
    client.docx.documentBlock.list({ path: { document_id: docToken } }, opts),
  ]);
...
async function writeDoc(
  ...
  const { blocks, firstLevelBlockIds } = await convertMarkdown(client, markdown, userAccessToken);
  if (blocks.length === 0) {
    return { success: true, blocks_deleted: deleted, blocks_added: 0, images_processed: 0 };
  }
  const sortedBlocks = sortBlocksByFirstLevel(blocks, firstLevelBlockIds);

  const { children: inserted, skipped } = await insertBlocks(
    client,
    docToken,
    sortedBlocks,
    undefined,
...
async function listBlocks(client: Lark.Client, docToken: string, userAccessToken?: string) {
    ...
    api.registerTool(
      {
        ...
        async execute(_toolCallId, params) {
          ...
          try {
            ...
            switch (p.action) {
              case "read":
                return json(await readDoc(client, p.doc_token, token));
              case "write":
                return json(
                  await writeDoc(
                    client,
              ...
              case "create":
                return json(await createDoc(client, p.title, p.folder_token, token));
              case "list_blocks":
                return json(await listBlocks(client, p.doc_token, token));
              case "get_block":
                return json(await getBlock(client, p.doc_token, p.block_id, token));
              case "update_block":
                return json(
                  await updateBlock(
                    client,
              ...
              case "delete_block":
                return json(await deleteBlock(client, p.doc_token, p.block_id, token));
              default:
