...
Single tool `feishu_wiki` for knowledge base operations, including creating spaces.
...
### Create Knowledge Space (NEW)

```json
{ "action": "create_space", "name": "服务器地图", "description": "服务器目录与配置地图" }
```

Creates a new wiki space. The creating bot/account becomes the owner/admin of that space by default (subject to tenant policy).
